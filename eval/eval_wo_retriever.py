...
parser.add_argument('--ckpt', type=str, default="/root/autodl-tmp/dataset/result/mosi/mr0.7_seed1234/imder-mosi.pth", help='Path to best_model.pth (MR=0.7)')
# parser.add_argument('--ckpt', type=str, default="/root/autodl-tmp/dataset/result/mosei/mr0.7_seed1234/imder-mosei.pth", help='Path to best_model.pth (MR=0.7)')
...
with torch.no_grad():
    for name, modal_idx in scenarios.items():
        ...
        for batch_data in dataloader:
            ...
            # zs = {k: torch.zeros_like(v).to(device) for k, v in batch_data['zs'].items()}
            zp = {k: v.to(device) for k, v in batch_data['zp'].items()}
            zp = {k: torch.zeros_like(v).to(device) for k, v in batch_data['zp'].items()}
